...
dc_begin(dc_t *parent, enum disir_context_type context_type, dc_t **child)
{
    ...
    case DISIR_CONTEXT_DEFAULT:
    case DISIR_CONTEXT_RESTRICTION:
        dx_crash_and_burn("%s - UNHANDLED CONTEXT TYPE: %s", __FUNCTION__, dx_context_type_string(context_type));
    case DISIR_CONTEXT_UNKNOWN:
...
dc_finalize(dc_t **context)
{
    ...
    case DISIR_CONTEXT_DOCUMENTATION:
        ...
        break;
    case DISIR_CONTEXT_CONFIG:
        dx_crash_and_burn("Context %s made to to switch it should never reach", dc_type_string(*context));
        break;
    case DISIR_CONTEXT_SCHEMA:
    case DISIR_CONTEXT_TEMPLATE:
    case DISIR_CONTEXT_GROUP:
