...
static enum disir_status
dio_toml_parse_table (struct disir_instance *instance, toml::Value& table,
                      struct disir_context *parent)
{
    ...
    for (const auto& kv : table.as<toml::Table> ())
    {
        ...
        case toml::Value::BOOL_TYPE:
        {
            status = dio_toml_unserialize_primitive (instance, kv.first.c_str(), kv.second, parent);
            break;
        }
        case toml::Value::INT_TYPE:
        {
        ...
        {
            disir_log_user (instance, "TOML: Type not implemented: %s", kv.second.typeAsString());
            // TODO: Just loop over this shizzle. Should probably call contents of this loop.
            status = DISIR_STATUS_INTERNAL_ERROR;
            break;
        }
        ...
        default:
        {
            disir_log_user (instance, "TOML: Type not handled: %s", kv.second.typeAsString ());
            status = DISIR_STATUS_INTERNAL_ERROR;
            break;
        }
        }
