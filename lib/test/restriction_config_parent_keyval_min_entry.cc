...
restriction_config_parent_keyval_min_entry (struct disir_mold **mold)
{
    ...
    // keyval
    status = dc_add_keyval_integer (context_mold, "keyval", 5, "complex doc", NULL,
                                  &context_keyval);
    if (status != DISIR_STATUS_OK)
        goto error;
    ...
    version.sv_major = 2;
    version.sv_minor = 0;
    status = dc_add_restriction_entries_min (context_keyval, 4, &version);
    if (status != DISIR_STATUS_OK)
        goto error;
