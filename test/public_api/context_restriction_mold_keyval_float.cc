...
class ContextRestrictionMoldKeyvalFloatTest : public testing::Test
{
    void SetUp()
    {
...
TEST_F (ContextRestrictionMoldKeyvalFloatTest,
        ...
        set_type_exclusive_value_enum_on_integer_keyval_shall_fail)
{
    ...
    ASSERT_STREQ ("cannot set restriction ENUM on FLOAT KEYVAL.", error);
}
...
TEST_F (ContextRestrictionMoldKeyvalFloatTest, restriction_type_numeric_set_range_shall_fail)
{
    status = dc_set_restriction_type (context_restriction, DISIR_RESTRICTION_EXC_VALUE_NUMERIC);
...
TEST_F (ContextRestrictionMoldKeyvalFloatTest, restriction_type_numeric_set_string_shall_fail)
{
    status = dc_set_restriction_type (context_restriction, DISIR_RESTRICTION_EXC_VALUE_NUMERIC);
    EXPECT_STATUS (DISIR_STATUS_OK, status);
...
TEST_F (ContextRestrictionMoldKeyvalFloatTest, restriction_type_numeric_set_numeric_shall_succeed)
{
    double value;
    ...
    EXPECT_EQ (6.78, value);
}

TEST_F (ContextRestrictionMoldKeyvalFloatTest, restriction_type_range_set_string_shall_fail)
{
    status = dc_set_restriction_type (context_restriction, DISIR_RESTRICTION_EXC_VALUE_RANGE);
    EXPECT_STATUS (DISIR_STATUS_OK, status);
...
TEST_F (ContextRestrictionMoldKeyvalFloatTest, restriction_type_range_set_numeric_shall_fail)
{
    status = dc_set_restriction_type (context_restriction, DISIR_RESTRICTION_EXC_VALUE_RANGE);
    EXPECT_STATUS (DISIR_STATUS_OK, status);
...
TEST_F (ContextRestrictionMoldKeyvalFloatTest, restriction_type_range_set_range_shall_suceed)
{
    double min, max;
