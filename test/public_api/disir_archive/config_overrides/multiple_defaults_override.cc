...
enum disir_status
multiple_defaults_1_2_differ_1_0 (struct disir_mold *mold, struct disir_config **config)
{
    enum disir_status status;
    struct disir_version version;
    struct disir_context *context_config;
    ...
    status = dc_set_value_string (context_keyval, "keyval_differ", strlen ("keyval_differ"));
    if (status != DISIR_STATUS_OK)
        goto error;
    ...
    status = dc_set_value_integer (context_keyval, 11);
    if (status != DISIR_STATUS_OK)
        goto error;
